...
The `ISignatureController` interface declares the functions that are implemented by `SignatureController` to enable [Meta transactions](../allow-my-users-to-manage-existing-domains/meta-transactions.md) for the `Registry` smart contract.
...
`SignatureUtil` is a helper smart contract. Its implementation is used to extend smart contracts that require [Meta Transactions](../allow-my-users-to-manage-existing-domains/meta-transactions.md).
...
`RelayTest` is used for testing relaying functionality for [Meta Transactions](../allow-my-users-to-manage-existing-domains/meta-transactions.md).
